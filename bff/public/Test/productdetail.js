  document.addEventListener("DOMContentLoaded", function () {
    const filterGroups = document.querySelectorAll(".container-sidebar-right-filter-list");

    filterGroups.forEach(group => {
      const items = group.querySelectorAll(".container-sidebar-right-filter-item");
    ...
    function updateHeaderText(categoryName) {
      const header = document.querySelector(".container-box-component--header--text");
      if (header) {
        header.textContent = categoryName ? `Sản phẩm ${categoryName}` : "Sản phẩm";
      }
    }
  });